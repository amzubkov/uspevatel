...
- **Navigation:** React Navigation. Bottom tabs: In / Day / Routine / 🎙 (voice command FAB, not a screen) / Control (merged Control+Later+Maybe with segment chips, `LaterScreen.tsx`) / Check / All
- **AI:** Ollama Cloud via `src/services/ollamaClient.ts`. Text default `glm-5.2`, vision `gemma4` (`DEFAULT_MODEL`/`VISION_MODEL`). `getOllamaModel()` auto-heals retired/paid saved models → default. `ollamaChatJson` has a 120s abort timeout. Features: workout planner, lab-test advisor, lab report photo parser, travel doc photo parser, food photo recognition (vision + glm-5.2 + qwen3.5, median per КБЖУ value), diet menu generation, voice command router (`voiceCommandService.ts`: intent JSON via gemma4 → food/task/sport/water/weight/note; unknown → inbox task), dictated-text proofread (`PolishTextButton`, gemma4)
- **Voice:** offline dictation via whisper.rn (`src/services/whisperService.ts`): PCM mic stream (@fugood/react-native-audio-pcm-stream, re-`init()` before EVERY start — native releases recorder on stop) → WAV → whisper.cpp, model ggml-base-q5_1 (~59MB) downloaded on first use to documents/whisper/. System SpeechRecognizer is dead on ColorOS (no default service, error 5/10, RECOGNIZE_SPEECH not exported) — don't retry that path. `VoiceInputButton` (day notes, QuickAddBar), `VoiceCommandButton` (tab bar FAB)
...
- **Sport/Exercise:** Weight types (0=bodyweight, 10=dumbbells, 100=barbell), calories per rep, priority 1-10 (higher = prefer in plans), workout programs (programs -> days -> day_exercises). Sport entry types include `walk` (steps; 1000 ≈ 0.42 kcal/kg). Gym kcal amortized ×3 (`SESSION_REST_AMORTIZATION` in utils/calories.ts — covers rest between sets); casual daily counters are NOT amortized. Cardio kcal shown in DayReview rows (orange, right)
...
- **Nutrition:** `nutrition_entries` (diary, per-100g macros + portion), `nutrition_plan` (editable day menu with `ingredients` JSON), `food_catalog` (offline product DB: ~100 seeded + 13k RU products imported per-device from OFF, source `data/off-ru-foods.zip`). Catalog search is in-memory (`foodDatabase.ts` cache) — SQLite lower()/LIKE is ASCII-only and breaks Cyrillic case matching. Online search: FatSecret (OAuth2 keys in SecureStore, IP-whitelisted at platform.fatsecret.com, badge FS) + Open Food Facts. "Чем добрать" suggestions run ONLY on button press (beam search blocks JS thread; per-food meta precomputed — ICU calls out of hot loops, was 7.9s on Hermes). Diary long-press: copy entry to today. Daily goals + diet split in settings (`nutritionGoal*`, `nutritionDiet`). Shopping list aggregates plan ingredients across days; bought state in `shoppingChecked` setting (shoppingStore). Fasting window = last meal today → first meal next day
