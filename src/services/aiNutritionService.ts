...
const num = (v: any, max = 100_000): number => {
...
function normalizeFood(parsed: ParsedFood, fallbackName = ''): ParsedFood {
  const food: ParsedFood = {
    name: String(parsed?.name || fallbackName || '').trim().slice(0, 300),
    amountGrams: num(parsed?.amountGrams, 10_000),
    kcalPer100: num(parsed?.kcalPer100, 5_000),
    proteinPer100: num(parsed?.proteinPer100, 500),
    fatPer100: num(parsed?.fatPer100, 500),
    carbsPer100: num(parsed?.carbsPer100, 500),
  };
...
export async function generateDietMenu(params: {
  ...
  if (!Number.isFinite(params.kcal) || params.kcal < 500 || params.kcal > 10_000) throw new Error('Некорректная цель по калориям');
  for (const macro of [params.protein, params.fat, params.carbs]) {
    if (!Number.isFinite(macro) || macro < 0 || macro > 1_000) throw new Error('Некорректная цель по БЖУ');
  }
...
Ответ — только JSON: {"meals":[{"mealType":"breakfast","name":"Овсянка на молоке с бананом","amountGrams":360,"kcalPer100":90,"proteinPer100":3.5,"fatPer100":2,"carbsPer100":15,"ingredients":[{"name":"овсяные хлопья","grams":60},{"name":"молоко","grams":200},{"name":"банан","grams":100}]}]}`;
  ...
  const items: MenuItem[] = meals.map((m) => ({
    ...
    amountGrams: num(m?.amountGrams, 10_000) || 100,
    kcalPer100: num(m?.kcalPer100, 5_000),
    proteinPer100: num(m?.proteinPer100, 500),
    fatPer100: num(m?.fatPer100, 500),
    carbsPer100: num(m?.carbsPer100, 500),
    ingredients: (Array.isArray(m?.ingredients) ? m.ingredients : [])
      .map((g: any) => ({ name: String(g?.name || '').trim().slice(0, 300), grams: num(g?.grams, 10_000) }))
