...
import { isValidDateStr, shiftDateStr, todayStr } from '../utils/date';
import { normalizeWorkoutPlan } from '../utils/aiValidation';
...
async function buildContext(targetDate: string, minutes: number): Promise<string> {
  ...
  const cutoff = shiftDateStr(todayStr(), -14);
  ...
  const recentDaily = dailyLogs
    ...
    .slice(0, 3)
    .map((d) => `${d.date}: сон ${d.sleepHours ?? '?'}ч${d.sleepQuality != null ? ` (кач-во ${d.sleepQuality}%)` : ''}${d.motivation != null ? `, мотивация ${d.motivation}/5` : ''}${d.dayRating != null ? `, день ${d.dayRating}/5` : ''}`)
    .join('\n') || 'нет данных';
...
export async function requestAiPlan(targetDate: string, minutes = 60): Promise<AiPlan> {
  if (!isValidDateStr(targetDate)) throw new Error('Некорректная дата тренировки');
  if (!Number.isFinite(minutes) || minutes < 15 || minutes > 240) throw new Error('Длительность тренировки должна быть от 15 до 240 минут');
  const raw: unknown = await ollamaChatJson({
  ...
  const plan = normalizeWorkoutPlan(raw, validIds);
