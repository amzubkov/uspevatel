...
export function MaybeScreen() {
  ...
  const categoryTasks = useMemo(() => tasks.filter((t) => t.category === 'MAYBE' && !t.completed), [tasks]);
      ...
      <div style={{ flex: 1, overflow: 'auto' }}>
        {filtered.length === 0 ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
            ...
            <span style={{ fontSize: 18, fontWeight: 600, color: c.textSecondary, marginTop: 12 }}>Нет идей и мечтаний</span>
          </div>
        ) : (
