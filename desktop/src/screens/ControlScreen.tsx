...
export function ControlScreen() {
  const { tasks, addTask, moveTask, completeTask, settings, projects } = useApp();
  ...
  const categoryTasks = useMemo(() => tasks.filter((t) => t.category === 'CONTROL' && !t.completed), [tasks]);
      ...
      <div style={{ flex: 1, overflow: 'auto' }}>
        {filtered.length === 0 ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
            ...
            <span style={{ fontSize: 18, fontWeight: 600, color: c.textSecondary, marginTop: 12 }}>Нет задач на контроле</span>
          </div>
        ) : (
          filtered.map((task, index) => (
            <div key={task.id} style={{ backgroundColor: index % 2 === 1 ? c.rowAlt : 'transparent' }}>
              <TaskCard
                ...
                actions={[
                  { label: 'Done', color: '#16A34A', onClick: () => completeTask(task.id) },
